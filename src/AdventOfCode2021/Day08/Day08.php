...
class Day08 extends AbstractSolver
{
    ...
    protected function mapLine(array $input)
    {
        ...
        $tests = [
            ...
            '9' => fn (string $pattern) => strlen($pattern) === 6 && $this->matchesAll($pattern, $this->signalsPerDigit[4]),
            // 0 is 6 segments, matching 1's segments (9 is already out)
            '0' => fn (string $pattern) => strlen($pattern) === 6 && $this->matchesAll($pattern, $this->signalsPerDigit[1]),
            // 6 is 6 segments, the only one left
            '6' => static fn (string $pattern) => strlen($pattern) === 6,
            // 3 is 5 segments and matches 1's segments
            '3' => fn (string $pattern) => strlen($pattern) === 5 && $this->matchesAll($pattern, $this->signalsPerDigit[1]),
            // 5 is 5 segments, and 9 has all the segments of 5
            '5' => fn (string $pattern) => strlen($pattern) === 5 && $this->matchesAll($this->signalsPerDigit[9], $pattern),
    ...
    protected function matchesAll(string $one, string $two): bool
    {
